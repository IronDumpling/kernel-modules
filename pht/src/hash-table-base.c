...
/* Hash Table Entry: hash_table_entry
...
/* Create New Hash Table: hash_table_base_create()
 * This function uses `calloc` to allocate dynamic memory, because it will be
 * zero intialized.
 * For every entry in our hash table, we also initialize the linked list.
 * If you add any fields to the structs you should initialize them
...
/* Get Linked List Entry: get_list_entry()
...
/* Contains key: hash_table_base_contains()
 ...
 * First we use our helper functions to see if this key already exists in the hash table.
 * If it exists we should update the value to the new value. We do not create a new entry in
 * this case because the key should only be in the hash table exactly one time.
...
/* This function uses frees all memory our hash table uses. First it goes
   ...
   we free each node in the linked list, by removing the first node
   from the list, freeing it, and doing that until the linked list is empty.
   After we free all the linked list nodes, we can free the hash table
